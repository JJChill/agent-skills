...
The one hook this fork's plugin does register is `hooks/probity-notice.sh` (via `hooks/hooks.json`): on `SessionStart` it warns when a repository below the project has a `probity.config.*` but the project root has none (Probity then enforces nothing), and prints nothing otherwise. Its regression test is `bash hooks/probity-notice-test.sh`.
