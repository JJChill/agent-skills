...
```bash
cmd skills add JJChill/agent-skills
```

In an interactive terminal this shows a multi-select so you can choose which of the 32 skills to install. Pipe/non-interactive invocations install all discovered skills.

**Install a specific skill:**

```bash
cmd skills add JJChill/agent-skills -s spec-driven-development
```

**User scope** (installs into `~/.commandcode/skills/`, available in every project):

```bash
cmd skills add JJChill/agent-skills --global
```

**Other supported forms:**

```bash
cmd skills add JJChill/agent-skills@main            # a specific branch
cmd skills add JJChill/agent-skills/skills/interview-me   # a specific path in the repo
cmd skills add JJChill/agent-skills --force         # overwrite / update if already installed
```
