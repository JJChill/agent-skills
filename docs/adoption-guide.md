...
1. Install the pack (`npx skills add JJChill/agent-skills`, or the native integration for your tool, see [getting-started.md](getting-started.md)).
...
- **Loading all 32 skills into every session.** It wastes context and dilutes the ones that matter. Load by phase; let the host's native router or `using-agent-skills` route, but not both.
