...
```bash
npx skills add JJChill/agent-skills            # install selected skills
...
```bash
npx skills add JJChill/agent-skills --skill spec-driven-development
```
...
Create an `AGENTS.md` in **your own project** root. This is the system prompt that tells OpenCode when and how to invoke the installed skills. Unlike the repo-scoped `AGENTS.md` in `JJChill/agent-skills`, this file belongs to your project and should be adapted to your stack.
...
> **Note:** The root `AGENTS.md` inside the `JJChill/agent-skills` repository is intended for contributors working on this repository and should not be copied into other projects. See [CONTRIBUTING.md](../CONTRIBUTING.md#repo-scoped-files).
...
1. Install the skills you need, either with `npx skills add JJChill/agent-skills` or by copying them from a clone of this repository into `.opencode/skills/` (project), `~/.config/opencode/skills/` (global), or a cross-compatible path such as `.claude/skills/` / `.agents/skills/`.
...
This keeps the reusable assets (skills) separate from the repository-specific configuration (the `JJChill/agent-skills` root `AGENTS.md`).
