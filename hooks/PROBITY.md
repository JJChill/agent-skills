...
| `withInverseScenarioGuidance()` (custom) | test-driven-development × acceptance-testing | Deterministic wrapper over the TDD gate: verdicts unchanged, but a violation on the test-control layer (acceptance composition root, fixtures) carries the inverse-scenario escape route in its deny text, so "observe a red first" never reads as "delete the control" |
